...
const condidates = [];
const numOfCandidates = Number(
  prompt('How many candidates you want to create?')
);

function createCandidates(numOfCandidates) {
  for (let i = 0; i < numOfCandidates; i++) {
    const candidateName = prompt(`Name for candidate ${i + 1}`);
    condidates.push({ name: candidateName, votes: 0 });
  }
}
createCandidates(numOfCandidates);

const numberOfVoters = Number(prompt('Enter how many voters?'));

function voting(condidates) {
  for (let i = 0; i < numberOfVoters; i++) {
    const vote = prompt(`who you will vote for? ${i + 1}`);

    const condidate = condidates.find(condidate => condidate.name === vote);

    if (condidate) condidate.votes++;
  }
}
voting(condidates);

function sortAndPrint(condidates) {
  const sorted = [...condidates].sort((a, b) => b.votes - a.votes);
  ...
  sorted.forEach(cond => {
    console.log(`${cond.name}: ${cond.votes} votes`);
  });
}

sortAndPrint(condidates);

// const originalArray = [2, 7, 4];
// const evenNumber = even(originalArray);
...
// console.log(`Copy (only even numbers): ${evenNumber.join(' ')}`);

// const names = ['Johnny', 'DeeDee', 'Joey', 'Marky'];

// function concat(names, separator = '') {
...
// document.querySelector('.names-list').textContent = concat(names);
...
//   const sides = Number(prompt('Enter the sides: '));

//   while (true) {
//     const roll = randomDice(sides);
//     numbers.push(roll);
//     if (roll === sides) break;
//   }
// }
...
// function printingEachRoll() {
//   numbers.forEach(roll => {
//     const li = document.createElement('li');
//     li.textContent = roll;
//     eachRoll.appendChild(li);
//   });
// }
